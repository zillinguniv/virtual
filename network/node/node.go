...
func (n *Node) initChain() {
	...
	// chdir for simplicity so core paths are relative
	...
	if !ok {
		...
		log.Printf("init CONFIG genesisTime=%d (%s)", onDiskCfg.GenesisTime, time.Unix(onDiskCfg.GenesisTime, 0).UTC())
	}

	// try load head via by-unix=0 (genesis) or existing by-hash head
	// if empty, create genesis
	// attempt: by-unix/0.json
	...
	if err != nil {
		...
		n.head = gen
		n.headBy = HeadFile{Hash: gen.Hash, Timestamp: gen.Header.Timestamp, VTCUnix: gen.Header.VTCUnix, PathUnix: up, PathHash: hp}
		writeJSON(core.BlocksRootSimple+"/HEAD.json", &n.headBy)
		log.Printf("genesis created hash=%s unix=%d", gen.Hash, gen.Header.VTCUnix)
	} else {
		// head is at least genesis; try load HEAD.json for exact head
		var hf HeadFile
		if ok, _ := readJSON(core.BlocksRootSimple+"/HEAD.json", &hf); ok {
			b, err := core.LoadBlock(hf.PathHash)
			if err == nil {
				n.head = b
				n.headBy = hf
			} else {
				n.head = g
				n.headBy = HeadFile{Hash: g.Hash, Timestamp: g.Header.Timestamp, VTCUnix: g.Header.VTCUnix}
			}
		} else {
			n.head = g
			n.headBy = HeadFile{Hash: g.Hash, Timestamp: g.Header.Timestamp, VTCUnix: g.Header.VTCUnix}
